...
if [ -f "cookies.txt" ]; then
    ...
    echo "✅ Using cookies for Wow.xxx authentication"
else
    COOKIE_FLAG=""
    echo "⚠️ No cookies found for Wow.xxx"
fi

# ── FUNCTIONS ────────────────────────────────
sanitize_name() {
    echo "$1" | sed 's/ /-/g' | sed 's/　/-/g' | tr -s '-' | sed 's/[^a-zA-Z0-9\-_.]/-/g'
}
...
download_video() {
    ...
    echo "Trying download method $METHOD for Wow.xxx..."
    
    COMMON_FLAGS="--merge-output-format mp4 --write-thumbnail --convert-thumbnails jpg --no-cache-dir --output ${TMP_DIR}/%(title)s.%(ext)s --no-part --no-playlist --retries 10 --fragment-retries 10 --no-check-certificates --concurrent-fragments 4 --buffer-size 16K --http-chunk-size 5M --limit-rate 3M --sleep-requests 2 --sleep-interval 3 --max-sleep-interval 10 --progress --newline"
    
    if [ "$QUALITY" = "audio" ]; then
        COMMON_FLAGS="--extract-audio --audio-format mp3 --audio-quality 0 --write-thumbnail --convert-thumbnails jpg --no-cache-dir --output ${TMP_DIR}/%(title)s.%(ext)s --no-part --no-playlist --retries 10 --fragment-retries 10 --no-check-certificates --concurrent-fragments 4 --buffer-size 16K --http-chunk-size 5M --limit-rate 3M --sleep-requests 2 --sleep-interval 3 --max-sleep-interval 10 --progress --newline"
    fi
    ...
    case $METHOD in
        1) yt-dlp $COOKIE_FLAG --proxy "socks5://127.0.0.1:1080" \
            ...
            --add-header "Referer: https://www.wow.xxx/" \
        ...
        6) yt-dlp $COOKIE_FLAG --proxy "socks5://127.0.0.1:1080" \
            --format "$FORMAT" $COMMON_FLAGS \
            --user-agent "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:109.0) Gecko/20100101 Firefox/121.0" \
            --add-header "Referer: https://www.wow.xxx/" \
            "$URL";;
            
        7) yt-dlp $COOKIE_FLAG --proxy "socks5://127.0.0.1:1080" \
            --format "$FORMAT" $COMMON_FLAGS \
...
echo "=========================================="
echo "✅ Wow.xxx download process completed!"
echo "=========================================="
